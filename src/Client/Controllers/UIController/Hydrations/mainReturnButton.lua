...
return function(payload: UIControllerD.UIPayload)
	...
	local isDisabled = Fusion.Computed(function()
		...
		if currentRoute == "/select-seat/" then
			return currentParams.section == nil and currentParams.seat == nil
		end

		if currentRoute == "/order-overview/" then
			return currentParams.holdStatus == nil
		end
	end)
	...
	Fusion.Hydrate(continueNav.Return)({
		[Fusion.OnEvent("MouseButton1Click")] = function()
			...
			if currentRoute == "/select-seat/" then
				...
				elseif currentParams.section then
					Producer.navigateTo("/select-seat/", {
						section = nil,
					})
				end
			...
			if currentRoute == "/order-overview/" then
				Producer.navigateTo("/select-seat/", {
					section = currentParams.seat:GetAttribute("Section"),
	...
	Fusion.Hydrate(continueNav.Return.TextLabel)({
